...
| ---------- | ----------------------------------- | ------------------ |
| Model      | Business logic + DB access          | Rails Built in ORM called Active Record,that can model relationships into sql db   |
| View       | Presentaion Logic: UI using HTML + CSS + Embedded Ruby (ERB) | `.html.erb` files  |
| Controller | Links user input → model + view , control all HTTP Req  | Controller classes |
...
│   ├── views/
│   ├── controllers/
│   ├── javascript/  # js dir config with hotwire, higly interactive frontend, without any unnecessary complexity of js component framework like react,angular,vue
│   ├── jobs/        # background workers
│   ├── mailers/     # email system
│   └── channels/    # websockets, for real time connections
├── db/              # schema & seeds
├── config/          # routes, initializers
├── Gemfile          # dependencies
├── Rakefile         # Makefile of Rails
```
...
- rails powerful cli
- Model name is plural by convention `Posts`
- then column:type
```bash
...
### `routes.rb`
- maps urls with controller

```ruby
resources :posts 
```

resources method Auto-creates 7 RESTful routes (index, show, new, create, edit, update, destroy)


### `posts_controller.rb`
- when user navigate one of the those routes, it triggers
```ruby
def index
  @posts = Post.all
end
```
- index will fetch all record in data base, by all method, provided automatically by model
- we can do data validation and joins etc. anything :0
- `@horses` instance variable, can be accessed even at html.erb file
